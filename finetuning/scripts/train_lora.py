...
def validate_required_config(config: dict[str, Any], allow_unresolved_runtime_paths: bool = False) -> list[str]:
    ...
    if config.get("load_in_4bit"):
        issues.append("train_lora.py does not support load_in_4bit=true. Use train_qlora.py for quantized training.")
    return issues
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a full-precision LoRA adapter on chat-formatted JSONL data.")
    parser.add_argument("--config", required=True, help="Path to the JSON or YAML config file.")
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> None:
    ...
    if config_issues and not args.dry_run:
        raise ValueError("\n".join(config_issues))
    ...
    if args.dry_run and (not output_dir_value or has_unresolved_placeholder(output_dir_value)):
        output_dir = ROOT / "runs" / "dry_run_lora"
    else:
